...
if uploaded_file is not None:
    ...
    try:
        ...
        # Show Matplotlib heatmap if Cartopy map is not selected
        if not show_cartopy_map:
            st.pyplot(fig)

        # Create a Cartopy map if Cartopy map is selected
        if show_cartopy_map:
            ...
            buffer.seek(0)
            st.image(buffer, caption='Rainfall Prediction Heatmap (Cartopy)', use_column_width=True)
            plt.close()
